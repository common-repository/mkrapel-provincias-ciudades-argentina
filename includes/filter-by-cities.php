...
function mkrapel_ar_filters_by_cities_method() {
    if ( ! class_exists( 'Filters_By_Cities_Method_AR' ) ) {
        class Filters_By_Cities_Method_AR extends WC_Shipping_Method {
            ...
            public function __construct($instance_id = 0) {
                ...
                $this->id                 	= 'mkrapel_ar_filters_by_cities_shipping_method';
                $this->instance_id 			= absint( $instance_id );
                $this->method_title       	= __( 'Envío por Ciudad AR');
                $this->method_description 	= __( 'Selecciona las Ciudades de la Provincia ingresado como <strong>"Zona de Envío"</strong>, recuerda que el <strong>Nombre del Tipo de Envío</strong> es el que se mostrará a tus compradores al momento de seleccionar el medio de pago deseado. <strong>Recuerda que seleccionar "No" como "Método de Envío Único" permites que puedan funcionar los otros tipos de Métodos de Envío establecidos con anterioridad</strong>');
                $this->supports = array(
                    'settings',
                    'shipping-zones',
                    'instance-settings'
            ...
            public function showCitiesRegions() {
                ...
                foreach ($zones as $zone){
                    if (strpos($zone->code, ':') !== false){
                        ...
                        $states = MkRapel_Provincia_Ciudad_AR::get_places( $place[0] );  /* Change for country */
